...
function datamosh() {
	fileSize=$(wc -c < "$file")
	headerSize=1000
	skip=$(gshuf -i "$headerSize"-"$fileSize" -n 1)
	count=$(gshuf -i 1-10 -n 1)
	for i in $(seq 1 $count);do byteStr=$byteStr'\x'$(gshuf -i 0-255 -n 1); done;
	printf $byteStr | dd of="$file" bs=1 seek=$skip count=$count conv=notrunc >/dev/null 2>&1
}

steps=$(gshuf -i 40-70 -n 1)
